...
namespace prostruct::core {
		...
		Args... computations)
	{
		...
		{
			// the decay_t is necessary to remove information about where the lambda was defined
			// i.e. if defined in main, std::get<0>(comp) would have type main()::lamda(T arg, ...)
			// the decay_t returns lambda(T arg, ...)
			constexpr size_t window_size = utils::lambda_properties<std::decay_t<decltype(std::get<0>(comp))>>::size;
			#pragma omp parallel
			for (std::size_t i = start; i < residues.size() - window_size + 1; ++i) {
				execute_tuple(comp,
